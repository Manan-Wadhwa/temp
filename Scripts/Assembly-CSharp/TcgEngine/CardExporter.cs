...
namespace TcgEngine
{
	public class CardExporter : MonoBehaviour
	{
		...
		private async void GenerateAll()
		{
			...
			export_texture = new Texture2D(width, height, TextureFormat.ARGB32, mipChain: false);
		...
		private void ExportCard(CardData card)
		{
			...
			export_texture.ReadPixels(new Rect(0f, 0f, width, height), 0, 0);
			byte[] bytes = export_texture.EncodeToPNG();
			string text = card.id + ".png";
			File.WriteAllBytes(export_path + "/" + text, bytes);
