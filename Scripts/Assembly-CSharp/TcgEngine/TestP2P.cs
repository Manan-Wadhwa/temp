...
namespace TcgEngine
{
	public class TestP2P : MonoBehaviour
	{
		...
		{
			error.text = "";
			if (await Authenticator.Get().Login(username.text, password.text))
			{
				UserData userData = await Authenticator.Get().LoadUserData();
				GameClient.player_settings.avatar = userData.GetAvatar();
				GameClient.player_settings.cardback = userData.GetCardback();
				deck_panel.Show();
				RefreshDeckList();
			}
			else
		...
		private void RefreshDeck(string tid)
		{
			...
			if (deck != null)
			{
				deck_preview.SetDeck(deck);
			}
			else if (deckData != null)
			{
				deck_preview.SetDeck(deckData);
			}
			else
			{
				deck_preview.Clear();
