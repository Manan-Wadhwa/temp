...
namespace TcgEngine.UI
{
	public class HoverTextBox : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			{
				base.transform.position = GameUI.MouseToWorld(Input.mousePosition);
				base.transform.rotation = Quaternion.LookRotation(Camera.main.transform.forward, Vector2.up);
				panel_left.SetVisible(base.transform.position.x > 0f);
				panel_right.SetVisible(base.transform.position.x <= 0f);
